...
def __get_dbgap_study_id(file: str, dbgap_study_id: str) -> str:
    if dbgap_study_id == "" or dbgap_study_id is None:
        ...
        print(f'{file} : {str(file).find("tar.gz") > 0 or str(file).find("fastq.gz") > 0} {dbgap_study_id}')
        if str(file).find("tar.gz") > 0 or str(file).find("fastq.gz") > 0:
...
def _build_dataframe(
    ...
    df["dbgap_study_id"] = df["filename"].apply(
